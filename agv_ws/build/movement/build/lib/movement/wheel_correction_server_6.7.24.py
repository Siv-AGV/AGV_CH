...
class SimpleController(Node):
    ...
    def command_callback(self, request, response):
        if not self.moving:  # Ensure it doesn't handle new requests if already moving
            msg = request.command
            # Calculate the resultant distance and angle based on x and y components
            self.target_distance = np.sqrt(msg.linear.x**2 + msg.linear.y**2)
            self.target_angle = math.atan2(msg.linear.y, msg.linear.x) + msg.angular.z
    ...
    def convert_pgv_values(self,pgv_msg):
        ...
        # print ("Tag detected: " + str(self.pgv_tag_detected))
    ...
    def pgv_callback(self, msg):
        ...
        self.correction_data = self.convert_pgv_values(msg)
    

    def apply_corrections(self):
        print ("Tag detected: " + str(self.pgv_tag_detected))
        if self.pgv_tag_detected == 1:
            x_position = self.correction_data.x_position 
            y_position = self.correction_data.y_position
            angle = math.radians(self.correction_data.angle)
    ...
    def update_movement(self):
        if self.moving:
            ...
            if self.correction_data:
                ...
                if abs(self.traveled_distance_local) >= 1.6 and abs(remaining_distance)>=1.0:
            ...
            if abs(remaining_distance) < 0.005 and abs(remaining_angle) < 0.05:
                self.current_wheel_speed = np.array([0.0, 0.0])
                self.moving = False
                self.timer.cancel()
