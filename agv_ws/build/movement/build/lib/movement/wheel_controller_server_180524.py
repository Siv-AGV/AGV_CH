...
class SimpleController(Node):
    ...
    def update_movement(self, response):
        if self.moving:
            ...
            self.current_wheel_speed_conv_R_s= int((( -1*self.wheel_speed[1]/(2*math.pi))*60*20 * 512 *10000)/1875)
            ...
            self.get_logger().info(f"Acceleration: {is_accelerating}, Deceleration: {is_decelerating}, Linear speed: {linear_speed}, DT: {dt} , RD: {remaining_distance}" )
